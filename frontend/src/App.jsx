...
// ── Global hover listener — lives at app level so it works on ALL pages ──────
function GlobalHoverListener() {
      ...
      timerRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (src.startsWith("blob:") || src.startsWith("data:")) {
            ...
            const api  = await describeImage(file);
            desc = api.data.description;
          } else {
            const api = await describeImageByUrl(src);
            desc = api.data.description;
          }
          setPanel({ desc, loading: false });
          speak(desc);
        } catch {
          const fallback =
  ...
  if (!hoverMode || !panel) return null;
  ...
  return (
    <>
      ...
      <div style={{
        ...
        zIndex: 9999, padding: "0 1rem 1.5rem",
        pointerEvents: "none",
      }}>
        <div style={{
          ...
          {panel.loading ? (
            <div style={{
              display: "flex", alignItems: "center",
              gap: "0.5rem", color: "#d97706", fontSize: "0.85rem",
            }}>
              <div style={{
  ...
  return (
    <AccessibilityProvider>
      <BrowserRouter>
        ...
        <Navbar />
        <GlobalHoverListener />  {/* ← lives here, persists across all pages */}
